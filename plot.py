...
class GeneratePlotData(object):
    """An object that will generate plots of friction varying some variable var1 over the range var1_range.
    
    To initialize the object, pass a cantilever, sample and experiment object, variable and variable range to the object. Example:
    ...
    def __init__(self, Cant, Samp, Expt, variable, variable_range):
        ...
        'friction': r'Friction $\Gamma_\mathrm{s}$ ($\mathrm{pN} \mathrm{s}/\mathrm{m}$)',
        'jitter': r'Jitter ($\mathrm{Hz}^2$)',
        'h':r'Sample Thickness $h$ (nm)',
        'power spectrum':r'$P^{\perp}_{\delta f_c}(f)$ ($\mathrm{Hz}^2 / \mathrm{Hz}$)',
        'f':r'$f$ (Hz)'
        }
        self.scales = {
        'rho':1e18,
    ...
    def _calc_variable_array(self, x_scale = 'log', n_pts =None):
        """Use the scale data to pick the appropriate spacing of the    points."""
        if n_pts == None:
            n_pts = self.n_pts
        if x_scale == 'log':
            _r = np.logspace(np.log10(self.variable_range[0]), np.log10(self.variable_range[1]), n_pts)
        elif x_scale == 'linear':
            _r = np.linspace(self.variable_range[0], self.variable_range[1], n_pts)
        else:
    ...
    def _pick_plot_func(self, x_scale, y_scale):
        """Convert the scale to a tuple, and then use a dictionary to pick the appropriate function for plotting. Returns the plot function"""
    ...
    def _make_sim_array(self):
        """Makes an array of simulations over the variable varied in the experiment, and also for multi_plot_var."""
    ...
    @autoassign
    def calc_plot_data(self, y_var, x_var, multi_plot_var, multi_plot_values, x_scale = 'log', n_pts = 50):
        """This plots y_var vs. x_var, over the range 'range' of 'variable' selected when the object was initialzied. y_var should be 'friction' or 'jitter'. Usage:
    ...
    def make_plot(self, figname = None, figsize = (3,3), fontsize = 10, linewidth = 2, xlim = None, ylim = None, properties = None):
        ...
        def reformat_properties(properties):
            ...
            into a format which can be looped over to apply them to individual lines."""
    ...
    def multi_power_spectrum(self, multi_plot_var, var_values, f_i = 1e-5, f_f = 1e6, n_pts = 100):
        """Plots the power spectrum, varying multi_plot_var over the values var_values. The user can also select the frequency range, which goes from an initial value of f_i to a final value of f_f. The user can also select the number of points at which to evaluate the power spectrum.
        
        
        TODO: Implement Marohn's protocol; save the data to a pickle file, and then read back the pickled data from a file in order to make the plot."""
        self.x_var = 'f'
