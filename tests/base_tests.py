...
def test_Cantilever_input():
    """Make sure that defining a cantilever with an incorrect geometry raises a ValueError."""
    c1 = Cantilever(geometry_c = 'not perpendicular or parallel')
...
def test_Cantilever_repr():
    """Make sure that the __repr__method regenerates an equivalent cantilever object."""
    c1 = Cantilever()
    assert c1 == eval(repr(c1))

class TestCantilever(unittest.TestCase):
    """Test the Cantilever properties to make sure they update automatically and raise errors when necessary."""
    def setUp(self):
        """Set up a default sample."""
        self.cantilever = Cantilever()
  
    def test_property_update(self):
        """Test that properties update appropriately after change parameters."""
        c = self.cantilever
        old_f_c = c.f_c
        c.f_c = old_f_c * 2
...
def test_Sample_V_g_rho_inconsistent():
    """Make sure that the Sample class gives a Value Error when invalid combinations of V_g and rho are passed to the function."""
    with assert_raises(ValueError):
        s1 = Sample(V_g = 10, rho = 122)
...
class TestSample(unittest.TestCase):
    """Test the sample class, specifically to make sure that its properties update automatically, and raise errors when necessary."""
    def setUp(self):
        """Set up a default sample."""
        self.sample = Sample()
...
"""Experiment currently has no tests; it is basically just a container at this point."""
